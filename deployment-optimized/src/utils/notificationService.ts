          ...
          prisma.notification.create({
            data: {
              ...
              message: description.length > 150 ? 
                ...
                description,
              type: NotificationType.NEW_PROMO,
    ...
    try {
      const notification = await prisma.notification.create({
        data: {
          ...
          message: `Selamat! Tiket "${ticketName}" telah berhasil dibuat untuk Anda. Silakan cek detail tiket di halaman My Ticket.`,
          type: NotificationType.TICKET_CLAIMED,
          relatedId: redeemHistoryId || null,
          relatedType: redeemHistoryId ? 'REDEEM_HISTORY' : 'TICKET',
          actionUrl: `/my-ticket`,
          actionText: 'Lihat Tiket'
        }
      });

      console.log(`Created ticket notification for member ${memberId}: ${ticketName}`);
      return notification;
    } catch (error) {
    ...
    try {
      const notification = await prisma.notification.create({
        data: {
          ...
          type: type === 'EARNED' ? NotificationType.POINTS_EARNED : NotificationType.POINTS_REDEEMED,
          actionUrl: '/profile',
