...
router.post('/verify-whatsapp-code', authMiddleware, async (req: any, res) => {
  try {
    ...
    // Update member's phone verification status
    const userId = req.user.uid as string;
    ...
    if (user?.member) {
      await prisma.member.update({
        ...
        data: { 
          ...
          phone: phone // Also update the phone number to the verified one
        }
      });
    }
...
router.post('/forgot-password', authLimiter, async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      return res.json({ success: true, message: 'Jika email terdaftar, kode reset password telah dikirim' });
    }
    ...
    const subject = 'Reset Password - The Lodge Family';
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0F4D39;">Reset Password</h2>
        <p>Halo ${userName},</p>
        <p>Anda telah meminta untuk mereset password akun The Lodge Family Anda.</p>
        <p>Berikut kode verifikasi untuk reset password:</p>
        <div style="font-size:24px; font-weight:bold; color:#0F4D39; text-align:center; padding:20px; background:#f5f5f5; border-radius:8px; margin:20px 0;">
          ${resetCode}
        </div>
        <p>Kode berlaku hingga ${resetTokenExpiry.toLocaleString('id-ID')}.</p>
        <p>Masukkan kode ini pada halaman reset password untuk melanjutkan.</p>
        <p><strong>Jika Anda tidak meminta reset password, abaikan email ini.</strong></p>
        <p>Terima kasih,<br>Tim The Lodge Family</p>
      </div>
    `;
    
    await sendEmail(email, subject, html);

    res.json({ success: true, message: 'Jika email terdaftar, kode reset password telah dikirim' });
  } catch (e) {
...
router.post('/verify-reset-code', authLimiter, async (req, res) => {
  try {
    const { email, resetCode } = req.body as { email: string; resetCode: string };
    if (!email || !resetCode) {
      return res.status(400).json({ message: 'Email dan kode reset harus diisi' });
    }
    ...
    if (user.resetToken !== resetCode || user.resetTokenExpiry < new Date()) {
      return res.status(400).json({ message: 'Kode reset tidak valid atau sudah kadaluarsa' });
    }

    res.json({ success: true, message: 'Kode reset valid' });
  } catch (e) {
...
router.post('/reset-password', authLimiter, async (req, res) => {
  try {
    ...
    if (!email || !resetCode || !newPassword) {
      return res.status(400).json({ message: 'Email, kode reset, dan password baru harus diisi' });
    }
