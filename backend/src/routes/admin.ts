...
router.post('/promos', adminAuth, upload.single('image'), async (req: any, res) => {
  try {
    const { title, description, startDate, endDate, type, pointsRequired, maxRedeem } = req.body as { title: string; description: string; startDate: string; endDate: string; type?: string; pointsRequired?: string; maxRedeem?: string };
    if (!title || !description || !startDate || !endDate) return res.status(400).json({ message: 'Missing fields' });

    let promoType: PromoType = PromoType.INFORMATION;
    if (type) {
      const upper = String(type).toUpperCase();
      if (!Object.values(PromoType).includes(upper as PromoType)) {
        return res.status(400).json({ message: 'Invalid promo type' });
      }
      promoType = upper as PromoType;
    }

    let pointsReq: number | undefined = undefined;
    if (promoType === PromoType.REDEEM_POINTS) {
      const parsed = parseInt(String(pointsRequired ?? '0'), 10);
      if (!parsed || parsed < 1) return res.status(400).json({ message: 'pointsRequired harus diisi dan minimal 1 untuk tipe Redeem Poin' });
      pointsReq = parsed;
    }

    let maxRedeemNum: number | undefined = undefined;
    if (promoType === PromoType.REDEEM_POINTS) {
      const parsedMax = parseInt(String(maxRedeem ?? '0'), 10);
      if (!parsedMax || parsedMax < 1) return res.status(400).json({ message: 'maxRedeem harus diisi dan minimal 1 untuk tipe Redeem Poin' });
      maxRedeemNum = parsedMax;
    }
    ...
    if (req.file) {
      if (cloudinary.config().cloud_name) {
        ...
        // fallback simpan ke /uploads dengan base URL dari request
        const uploadsDir = path.join(process.cwd(), 'uploads');
        try { fs.mkdirSync(uploadsDir, { recursive: true }); } catch {}
...
router.put('/promos/:id', adminAuth, upload.single('image'), async (req: any, res) => {
  try {
    ...
    if (typeof pointsRequired !== 'undefined') {
      if ((data.type ?? undefined) === PromoType.REDEEM_POINTS || typeof data.type === 'undefined') {
        const parsed = parseInt(String(pointsRequired ?? '0'), 10);
        if (!parsed || parsed < 1) return res.status(400).json({ message: 'pointsRequired harus diisi dan minimal 1 untuk tipe Redeem Poin' });
        data.pointsRequired = parsed;
      } else {
        // jika bukan tipe redeem points, nolkan
        data.pointsRequired = null;
      }
    }

    if (typeof maxRedeem !== 'undefined') {
      if ((data.type ?? undefined) === PromoType.REDEEM_POINTS || typeof data.type === 'undefined') {
        const parsedMax = parseInt(String(maxRedeem ?? '0'), 10);
        if (!parsedMax || parsedMax < 1) return res.status(400).json({ message: 'maxRedeem harus diisi dan minimal 1 untuk tipe Redeem Poin' });
        data.maxRedeem = parsedMax;
      } else {
        data.maxRedeem = null;
      }
    }

    const updated = await prisma.promo.update({ where: { id: req.params.id }, data });
    res.json(updated);
  } catch (e: any) {
...
router.get('/promos', adminAuth, async (_req, res) => {
  try {
    ...
    const targetBase = `${_req.protocol}://${_req.get('host')}`;
    const enriched = list.map((p) => {
      const now = new Date();
      const isActive = now >= p.startDate && now <= p.endDate;
      let fixedImageUrl = p.imageUrl;
      try {
      ...
      return { ...p, status: isActive ? 'AKTIF' : 'KEDALUWARSA', imageUrl: fixedImageUrl };
    });
    res.json(enriched);
  } catch (e: any) {
