    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="flex-1 min-w-[200px]">
          ...
          <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Nama atau email..." className="w-full px-3 py-2 rounded border border-slate-300 bg-white" />
        </div>
        <div>
          <label className="block text-xs text-slate-600 mb-1">Filter Role</label>
          <select value={filterRole} onChange={(e) => setFilterRole(e.target.value as AdminRole | "ALL")} className="px-3 py-2 rounded border border-slate-300 bg-white">
        ...
        <div>
          ...
          <select value={filterActive} onChange={(e) => setFilterActive(e.target.value as "ALL" | "ACTIVE" | "INACTIVE")} className="px-3 py-2 rounded border border-slate-300 bg-white">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button onClick={() => { setFilterRole("ALL"); setFilterActive("ALL"); setSearchQuery(""); }} className="px-3 py-2 rounded-md bg-slate-200 text-slate-700">Reset</button>
          <button onClick={() => {
            try {
          ...
          <tbody className="divide-y divide-slate-200">
            ...
            {paginated.map((a) => (
              <tr key={a.id}>
                ...
                <td className="px-4 py-2 text-xs">
                  <span className={`inline-flex items-center px-2 py-1 rounded text-[10px] font-medium ${a.isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-700"}`}>{a.isActive ? "Aktif" : "Nonaktif"}</span>
                </td>
                <td className="px-4 py-2 text-xs">{a.createdAt ? new Date(a.createdAt).toLocaleString("id-ID") : "-"}</td>
                <td className="px-4 py-2 text-xs">
                  <div className="flex items-center gap-2">
                    <button onClick={() => openEdit(a)} className="px-2 py-1 rounded bg-slate-200 text-slate-700 hover:bg-slate-300">Edit</button>
      ...
      <div className="px-4 py-2 flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="px-2 py-1 rounded bg-slate-200 text-slate-700 disabled:opacity-50">Prev</button>
          <span className="text-xs">Hal {page} / {totalPages}</span>
          <button disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))} className="px-2 py-1 rounded bg-slate-200 text-slate-700 disabled:opacity-50">Next</button>
          <select value={pageSize} onChange={(e) => setPageSize(parseInt(e.target.value) || 10)} className="px-2 py-1 rounded border border-slate-300 bg-white text-xs">
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-lg w-full max-w-md p-4 border border-slate-200" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full px-3 py-2 rounded border border-slate-300 bg-white" />
              </div>
              <div>
                <label className="block text-xs text-slate-600 mb-1">Email</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-3 py-2 rounded border border-slate-300 bg-white" />
              </div>
              <div>
                <label className="block text-xs text-slate-600 mb-1">Password</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-2 rounded border border-slate-300 bg-white" />
              </div>
              <div>
                <label className="block text-xs text-slate-600 mb-1">Role</label>
                <select value={adminRole} onChange={(e) => setAdminRole(e.target.value as AdminRole)} className="w-full px-3 py-2 rounded border border-slate-300 bg-white">
                  <option value="CASHIER">CASHIER</option>
                  <option value="MODERATOR">MODERATOR</option>
                  <option value="OWNER">OWNER</option>
                  <option value="SUPER_ADMIN">SUPER_ADMIN</option>
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setShowForm(false)} className="px-3 py-2 rounded-md bg-slate-200 text-slate-700">Batal</button>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowEdit(false)}>
          <div className="bg-white rounded-lg w-full max-w-md p-4 border border-slate-200" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-slate-600 mb-1">Nama Lengkap</label>
                <input value={editFullName} onChange={(e) => setEditFullName(e.target.value)} className="w-full px-3 py-2 rounded border border-slate-300 bg-white" />
              </div>
              <div>
                <label className="block text-xs text-slate-600 mb-1">Email</label>
                <input type="email" value={editEmail} onChange={(e) => setEditEmail(e.target.value)} className="w-full px-3 py-2 rounded border border-slate-300 bg-white" />
              </div>
              <div>
                <label className="block text-xs text-slate-600 mb-1">Role</label>
                <select value={editRole} onChange={(e) => setEditRole(e.target.value as AdminRole)} className="w-full px-3 py-2 rounded border border-slate-300 bg-white">
                  <option value="CASHIER">CASHIER</option>
                  <option value="MODERATOR">MODERATOR</option>
                  <option value="OWNER">OWNER</option>
                  <option value="SUPER_ADMIN">SUPER_ADMIN</option>
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setShowEdit(false)} className="px-3 py-2 rounded-md bg-slate-200 text-slate-700">Batal</button>
          ...
          <div className="bg-white rounded-lg w-full max-w-md p-4 border border-slate-200" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="password" value={resetPassword} onChange={(e) => setResetPassword(e.target.value)} className="w-full px-3 py-2 rounded border border-slate-300 bg-white" />
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setShowReset(false)} className="px-3 py-2 rounded-md bg-slate-200 text-slate-700">Batal</button>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowDelete(false)}>
          <div className="bg-white rounded-lg w-full max-w-md p-4 border border-slate-200" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2">
              ...
              <input value={deleteConfirmText} onChange={(e) => setDeleteConfirmText(e.target.value)} placeholder={deleteAdminTarget.email} className="w-full px-3 py-2 rounded border border-slate-300 bg-white" />
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setShowDelete(false)} className="px-3 py-2 rounded-md bg-slate-200 text-slate-700">Batal</button>
